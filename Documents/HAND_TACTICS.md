...
プレイヤーは32枚のデッキを構築し、毎ターン「じゃんけん」を行いながら、マナポイント(MP)を消費してカードをプレイし、相手リーダーのHPを0にすることを目指します。  
...
本仕様書およびゲーム内で使用されるわかりづらい用語を解説します。

| 用語 | 説明 |
| :--- | :--- |
| **リーダー (Leader)** | プレイヤーの分身となるキャラクター。初期HPは20。HPが0になると敗北となります。 |
...
| **Lose Bonus** | ターン開始時のじゃんけんに敗北した際に発動する追加効果。 |
| **Hand Synchro** | ターン開始時に選んだ手と、使用するカードの属性が一致した際に得られるボーナス。 |
...
* **プレイヤー数** 2人
* **勝利条件:** 相手リーダーのHPを **0** にする  
* **敗北条件:** 自分リーダーのHPが **0** になる
* **初期HP:** 20
* **手札上限** 6枚(6枚を超えて引かない)  
* **デッキ上限** 32枚
* **フィールド上限** 6枚ずつ
* **リソース:** マナポイント(MP)。毎ターン最大値が1増え、全回復する（最大10）
* **カードプレイ** マナポイント(MP)を消費して、スペルやモンスターをプレイし、相手のモンスターやリーダーを攻撃して勝利を目指す
...
毎ターン開始時、プレイヤーは グー チョキ パー のいずれかを選択する。この結果に応じて、そのターン中カードに影響を与える場合がある。

* **Win Bonus (勝利時):** じゃんけんに勝っていると、カードが強化される（攻撃力アップ、ドローなど）。  
* **Lose Bonus (敗北時):** 負けている時のみ発動する強力なカウンター効果（スピードアタッカー化、トークン召喚など）。  
* **Hand Synchro (属性一致):** 自分が選んだ「手」と同じ属性のカードはボーナスを得る(属性一致且つ勝利していれば、その手と同じ属性を持つカードはコスト-1)。

#### **2.3.2. バトル時の属性相性**

モンスター同士の戦闘でもじゃんけん属性（✊✌️✋）が勝敗を分ける。

* **有利 (例: ✊ vs ✌️):** ✊が相手を一方的に破壊する。  
* **不利 (例: ✊ vs ✋):** ✊のカードが一方的に破壊される。  
* **あいこ:** 攻撃力(Power)が高い方が勝つ。同じなら相打ち(両方破壊)。
