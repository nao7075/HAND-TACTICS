...
「HAND TACTICS」は、デジタルカードゲームの戦略性に、誰もが知る「じゃんけん」の要素を融合させた1vs1の対戦型カードゲームです。  
プレイヤーは32枚のデッキを構築し、毎ターン「じゃんけん」を行いながら、マナを消費してカードをプレイし、相手リーダーのライフを0にすることを目指します。  

単なる属性相性だけでなく、「毎ターンのじゃんけん勝負」の結果がカードの強さを変化させる独自のシステムにより、高度な読み合いと劇的な逆転要素を提供します。
...
本仕様書およびゲーム内で使用されるわかりずらい用語を解説します。

| 用語 | 説明 |
| :--- | :--- |
| **リーダー (Leader)** | プレイヤーの分身となるキャラクター。初期ライフは20。ライフが0になると敗北となります。 |
| **マナポイント (MP)** | カードを使用するために必要なコスト。毎ターン開始時に最大値が1増え、全回復します（最大10）。 |
| **モンスター (Monster)** | フィールドに召喚され、戦闘を行うカード。攻撃力(Power)とじゃんけん属性を持ちます。 |
| **スペル (Spell)** | 使用すると即座に効果を発揮し、場に残らない使い切りのカード。 |
| **じゃんけん属性** | グー、チョキ、パーの3すくみの属性。モンスターやターン開始時の手に設定されています。 |
| **手 (Hand)** |  ターン開始時にプレイヤーが選択するじゃんけんの手のこと。 |
| **Win Bonus** | ターン開始時のじゃんけんに勝利した際に発動する追加効果。 |
| **Lose Bonus** | ターン開始時のじゃんけんに敗北した際に発動する追加効果（逆転要素）。 |
| **Hand Synchro** | ターン開始時に選んだ手と、使用するカードの属性が一致した際に得られるボーナス（主にコスト軽減）。 |
| **スピードアタッカー** | プレイしたターンに攻撃できるモンスターに与えられる効果。

### **1.5. カードの見方**

| カード構成 | 詳細 |
| :--- | :--- |
| ![alt text](DCG_game_images/card_explain.png) | **1:カード名**<br>このカードのカード名。<br><br>**2:コスト**<br>この数字分のMPを消費してカードをプレイする。<br><br>**3:カードイラスト**<br>このカードのイラスト。<br><br>**4:じゃんけん属性**<br>このカードのじゃんけん属性。この属性によってカードの色が変わる。グーは赤、チョキは緑、パーは青。<br><br>**5:攻撃力(Power)**<br>このカードの攻撃力(Power)。 |
...
* **デッキ上限** 32枚
* **フィールド上限** 6枚ずつ
* **リソース:** マナポイント（MP）。毎ターン最大値が1増え、全回復する（最大10）
* **カードプレイ** マナポイントを消費して、スペルやモンスターをプレイし、相手のモンスターやリーダーを攻撃して勝利を目指す


### **3.3. コアメカニクス（じゃんけんバトル）**
...
### **3.5. オンライン機能 PUN2(Photon Unity Networking 2)**
...
* **全体レイアウト:**  
  ...
  * **サイドバー (右側):**  
    ...
    * * **マナポイント表示:** TurnEndボタンを挟んで配置。赤（敵）と青（自分）のダイヤ型アイコンで数値 (10/10など) を表示。  
  * **サイドバー (左側):**  
...
| **15** | ダーヴィン | ✊ | Monster | 3/2 | card15.jpg | マナポイント最大値+1 (Mana Boost) |
...
| **44** | 神の恵み | ✌️ | Spell | 3/- | card44.jpg | マナポイント最大値+1 |
